...
Elle n'a pas été implémenté, mais la base sur laquelle j'étais partie
...
\medskip
Les valeurs des trames à envoyer sont les mêmes jusqu'à un appuis sur
...
Les différentes commandes viennent de l'IHM \emph{(voir
  ...
  un reset)}et sont envoyés en continue tant que pas de nouvelle pression sur bouton Central.
...
Une fois que le sustème d'enclenchement a vérifé que toutes les
...
Cette partie n'a pas été implementée mais voici comment elle aurait du
fonctionner.

Chaque itinairaire aurait été un tableau de n instructions.
Le tableau aurait contenu une suite de valeur qu'il aurait fallut
avoir sur le circuit pour passer à la suivante, c'est à dire l'état
des aiguilles et la valeur des Tracks circuits occupé.

Pour l'aiguillage il y aurait 3 états pour chauques aiguillages,
\begin{itemize}
...
Pareil pour les Tracks Circuits

On tenterait d'obtenir les deux valeurs en même temps pour passer à la
case suivante du tableau, une fois l'itineraire completé, on
allumerait la led correspondant au switch pour signaler la fin.
